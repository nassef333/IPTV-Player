...
// ─── movie card ───────────────────────────────────────────────────────────────
function MovieCard({ movie, index, onClose }: {
  ...
  const handleClick = () => {
    onClose();
    router.push(`/movie/${movie.stream_id}`);
  };
    ...
    >
      <div className="relative w-full aspect-[2/3] rounded-xl overflow-hidden bg-[#1c1c1c]">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3
          ...
          <p className="text-xs font-semibold line-clamp-2 leading-tight">{movie.name}</p>
        </div>
      </div>

      <div className="px-0.5">
        <p className="text-xs font-medium line-clamp-2 text-gray-300 leading-snug">{movie.name}</p>
        {movie.rating_5based > 0 && <div className="mt-1"><Stars value={movie.rating_5based} /></div>}
      </div>
    </div>
  );
}
      ...
      >
        ...
        <header className="sticky top-0 z-10 bg-[#0e0e0e]/95 backdrop-blur-lg border-b border-white/5">
          <div className="px-6 py-4 flex items-center gap-3">
            ...
            <div className="relative flex-1">
              ...
              <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-xl transition"
                >
                  Search
                </button>
              </div>
            ...
            >
              ...
              {activeFilters > 0 && (
                <span className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-red-500 rounded-full
                  ...
                  {sortOpen && (
                    <div className="absolute top-full left-0 mt-2 w-44 bg-[#1c1c1c]
                      ...
                      {SORT_OPTIONS.map((opt) => (
                        ...
                        >
                          {sort === opt.value && <span className="mr-2 text-red-500">✓</span>}
                          {opt.label}
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="absolute top-full left-0 mt-2 w-32 bg-[#1c1c1c]
                      ...
                      {YEAR_OPTIONS.map((opt, i) => (
                        <button key={opt.value} onClick={() => { setYearIdx(i); setYearOpen(false); }}
                          ...
                          {yearIdx === i && <span className="mr-2 text-red-500">✓</span>}
                          {opt.label}
                        </button>
                      ))}
                    </div>
        ...
        <div className="px-6 pb-12">
          ...
          {!fetching && !hasSearched && (
            <div className="flex flex-col items-center justify-center py-24 gap-3 text-gray-700">
              <Film size={48} />
              <p className="text-lg">Search for movies</p>
              <p className="text-sm text-gray-500">Enter a movie name and click Search</p>
            </div>
          )}

          {!fetching && hasSearched && pageItems.length > 0 && (
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 2xl:grid-cols-8 gap-4">
              {pageItems.map((movie, i) => (
                <MovieCard key={movie.stream_id} movie={movie} index={i} onClose={onClose} />
              ))}
            </div>
          )}
        </div>
